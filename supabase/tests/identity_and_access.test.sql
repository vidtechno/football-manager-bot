...
BEGIN
    BEGIN
        ...
        RAISE EXCEPTION 'Test Failed: Duplicate telegram_user_id was allowed.';
...
BEGIN
    BEGIN
        ...
        RAISE EXCEPTION 'Test Failed: Non-positive telegram_user_id was allowed.';
...
BEGIN
    BEGIN
        INSERT INTO public.managers (telegram_user_id, manager_name)
        VALUES (100002, 'AB');
        RAISE EXCEPTION 'Test Failed: Short manager_name (<3 chars) was allowed.';
    EXCEPTION WHEN check_violation THEN
        -- Expected behavior
    END;
END;
...
BEGIN
    ...
    BEGIN
        ...
        RAISE EXCEPTION 'Test Failed: Duplicate manager_id in manager_profiles was allowed.';
    EXCEPTION WHEN unique_violation THEN
        -- Expected behavior
    END;
    ...
    BEGIN
        ...
        RAISE EXCEPTION 'Test Failed: Negative reputation_score was allowed.';
...
BEGIN
    ...
    BEGIN
        ...
        RAISE EXCEPTION 'Test Failed: Multiple active blocks for one manager were allowed.';
    EXCEPTION WHEN unique_violation THEN
        -- Expected behavior
    END;
    ...
    BEGIN
        ...
        RAISE EXCEPTION 'Test Failed: unblocked_at earlier than blocked_at was allowed.';
...
BEGIN
    ...
    BEGIN
        ...
        RAISE EXCEPTION 'Test Failed: UPDATE on admin_audit_logs was allowed.';
    ...
    BEGIN
        ...
        RAISE EXCEPTION 'Test Failed: DELETE on admin_audit_logs was allowed.';
    EXCEPTION WHEN raise_exception THEN
        -- Expected behavior
    END;
END;
