...
- [ ] **4. Supabase poydevori va migratsiyalar (Supabase foundation and migrations)**
  ...
  - [x] **4C. Supabase Global Shablonlar domen migratsiyasi (`create_global_club_and_player_templates`):** Tayyorlash va remote masofaviy Supabase loyihasiga (`cxuqmfvnrzsrafjhoggu`) muvaffaqiyatli push qilish yakunlandi (20260818210748). `club_templates`, `club_template_versions`, `player_templates`, `player_template_positions`, `player_template_versions` jadvallari, 20 ta boshlang'ich klub slaglari, `enum_player_position` va to'liq normalizatsiya qilingan versiyalash modeli joylashtirildi. Anonymous API ruxsat taqiqi (Status 401) barcha 5 ta jadval bo'yicha sinovdan o'tdi.
  ...
  - [x] **4E. Joriy 20 ta klub tarkiblari, narxlari va baholashlarining versiyalangan ma'lumotlar to'plami (`current_squads_and_player_ratings`):** 20 ta top-klub uchun 567 ta birinchi jamoa futbolchilari, ularning bozor narxlari (€16.75 mlrd umumiy qiymat), pozitsiyalari va atributlari `data/football/2026-08-19/` katalogida shakllantirildi. `src/data/validate-dataset.ts` va `src/data/generate-seed-sql.ts` orqali Zod validatsiyasi va takrorlanuvchan `supabase/seed.sql` shakllantirildi. Vitest testlari to'liq o'tdi.
  - [x] **4F. Tarkib sig'imini 18-30 gacha oshirish va Afsonalar Bozori (Legend Transfers) poydevori (`squad_size_30_and_legend_transfers`):** Tarkib sig'imi chegarasi 18-30 ga o'zgartirildi hamda pozitsiya minimal talablari (2 GK, 6 DEF, 6 MID, 4 FWD) o'rnatildi. Global `legend_templates` va liganing har bir nusxasi uchun ajratilgan `league_legend_market` jadvallari, atomar tranzaktsiyaviy `purchase_league_legend` RPC funksiyasi, Zod validator va pgTAP va Vitest testlari shakllantirildi (20260819140000).
  - [x] **4G. Transfer budjeti paketi, admin tasdiqlash oqimi, kunlik 3-tur limiti va solo liganing butunlay o'chirilishi (`transfer_budget_packages_and_league_controls`):** Transfer budjeti integratsiyasi (€100m-€500m), 5 ta real to'lov paketi (`transfer_budget_packages`), buyurtma kodlari, atomar admin tasdiqlashi (`approve_transfer_budget_purchase_request`), Toshkent vaqti bo'yicha kunlik 3-tur limiti (`check_daily_round_limit`), va solo liganing tranzaktsiyaviy o'chirilishi (`delete_solo_league`) migratsiyasi (20260819150000) va testlari shakllantirildi.
  - [x] **4H. Telegram Bot ishga tushirish integratsiyasi va Xavfsizlikni kuchaytirish (`harden_rpc_security_and_round_limits`):** RPC xavfsizligi va RLS grant cheklovlari (`service_role` uchun ajratish, `SET search_path = public`), Telegram inline tugmalari (`💰 Transfer budjetini oshirish`, `💬 Admin bilan bog‘lanish`, `💳 Transfer budjeti buyurtmalari`, `🗑 Ligani butunlay o‘chirish`), deep link bog'lanishi (`@diyorbek_anorboyev`), kanonik tur o'tkazish tranzaksiyasiga kunlik 3-tur limitini integratsiyalash (`execute_league_round`), va 25 ta Vitest unit va pgTAP testlari muvaffaqiyatli topshirildi (20260819160000).
  - [x] **4I. Admin callback yo'nalishlari va Afsonalar Bozori (Legend Transfers) to'liq ma'lumotlar to'plami (`validated_legend_transfers_dataset`):** Admin tasdiqlash va rad etish callback'lari (`adm_app_req`, `adm_rej_req`) bot routeriga kiritildi, barcha 15 pozitsiyani qamrab oluvchi 60 ta afsonaviy futbolchining audittan o'tgan ma'lumotlar to'plami (`data/football/legends/legends.json`), manbalar fayli (`sources.json`), Zod validatori hamda `supabase/seed.sql` shakllantirildi. 32 ta Vitest unit test va pgTAP testlari to'liq topshirildi.
  - [x] **4J. Doimiy Transfer Bozori (`permanent_player_transfer_market`):** Tur cheklovlarisiz 24/7 ishlovchi transfer bozori, `league_transfer_listings` jadvali, sotuvchida 4 ta active e'lon cheklovi, insoniy xarid RPC (`purchase_player_transfer_listing`), 24 soatdan keyingi bot xaridlari RPC (`process_bot_transfer_reviews`), transferlar tarixi view, Uzbek telegram UI va testlar yaratildi (20260820000000).
  - [x] **5. Avtomatik Klub Daromadlari va Yagona Global Homiy Kanali (`automatic_club_income_and_global_sponsor`):** Har bir yakunlangan turdan keyin avtomatik daromadlar (€2.5 mln homiylik, €1.5 mln g'alaba bonusi, €500k durrang bonusi, €1.0M-€1.5M stadion daromadi), yagona global homiy Telegram kanali (`global_sponsor_settings`), 6 soatlik keshli a'zolik tekshiruvi (`manager_sponsor_verifications`), botlar uchun avtomatik homiylik, admin boshqaruvi (`📢 Homiy kanal boshqaruvi`), foydalanuvchi daromadlar tarixi (`📊 Klub daromadlari`) va idempotensiya sinovlari topshirildi (20260821000000).
